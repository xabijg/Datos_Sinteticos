...
def get_classifiers():
    classifiers = {
        ...
        "gbforest": GradientBoostingClassifier(n_estimators=500, random_state=42),
...
for dataset in datasets:
    ...
    for c_value, files in files_by_c.items():
        ...
        for split_path in files:
            ...
            base_name_clean = re.sub(r"_C[\d.]+(?=\.csv)", "", base_name).replace("ranking_", "")
            base_name2_clean = re.sub(r"_C[\d.]+(?=\.csv)", "", base_name2).replace("ranking_", "")
            ...
            target_col = next((col for col in ["Gallstone Status", "Class", "class", "Label", "HONG", "BACT", "VIRUS"] if col in df_train.columns), None)
            if not target_col:
            ...
            for clf_name in base_classifiers.keys():
                ...
                max_feats = len(ranking_df)
                if MAX_FEATURES_TO_TEST is not None:
                    max_feats = min(MAX_FEATURES_TO_TEST, max_feats)

                for n in range(1, max_feats + 1):
                    selected_features = ranking_df["feature"].iloc[:n].values
                    X_train = X_train_all[selected_features]
                    X_test = X_test_all[selected_features]

                    clf = clone(base_classifiers[clf_name])  #  CLONAMOS EL CLASIFICADOR ANTES DE CADA USO
