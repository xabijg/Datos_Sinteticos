...
#         selected_features = X_train.columns[selector.support_].tolist()# Lista con los nombres de las columnas que el selector vio utiles
#
...
from sklearn.feature_selection import RFECV
from sklearn.model_selection import train_test_split, StratifiedKFold
# from sklearn.preprocessing import RobustScaler  # Ya no es necesario importar RobustScaler
...
def crossValidation_scall(base_name):
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(df, df[target_column]), start=1):
        ...
        model_final = ExtraTreesClassifier(random_state=42, n_jobs=-1)
        ...
        # Guardamos directamente sin escalar
        df_train_fold.to_csv(f"{folds_output_dir}/train_fold_{fold}_{basename}.csv", index=False)
        df_val_fold.to_csv(f"{folds_output_dir}/val_fold_{fold}_{basename}.csv", index=False)
